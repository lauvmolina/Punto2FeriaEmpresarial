...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        ...
        for (int i=0;i<puestos.length;i++){
            ...
            if(puestos[i].estaOcupado()){
                Empresa e= darempresaporNombre(puestos[i].darNombreEmpresa());
                        switch (puestos[i].darZona())
                        {
                            case Puesto.ZONA_NORTE:
                                    contNorte+=e.darNumeroPersonasAsistentes();
                                    if(contNorte>maxNum){
                                        maxNum=contNorte;
                                        zonaMax=puestos[i].darZona();
                                    }
                                break;
                            case Puesto.ZONA_ORIENTE:
                                contOriente+=e.darNumeroPersonasAsistentes();
                                if(contOriente>maxNum){
                                    maxNum=contOriente;
                                    zonaMax=puestos[i].darZona();
                                }
                                break;
                            case Puesto.ZONA_SUR:
                                ...
                                contSur+=e.darNumeroPersonasAsistentes();
                                if(contSur>maxNum){
                                    maxNum=contSur;
                                    zonaMax=puestos[i].darZona();
                                }
                            ...
                            case Puesto.ZONA_OCCIDENTE:
                                ...
                                contOccidente+=e.darNumeroPersonasAsistentes();
                                if(contOccidente>maxNum){
                                    maxNum=contOccidente;
                                    zonaMax=puestos[i].darZona();
                                }
                                break;
                            case Puesto.ZONA_CENTRO:
                                contCentro+=e.darNumeroPersonasAsistentes();
                                if(contCentro>maxNum){
                                    maxNum=contCentro;
                                    zonaMax=puestos[i].darZona();
                                }
                                break;
        ...
        System.out.println(contCentro);
        System.out.println(contOriente);
        System.out.println(contOccidente);
        System.out.println(contSur);
        System.out.println(contNorte);
     ...
     * @return respuesta2
     */
    public Empresa darempresaporNombre(String nombre) {

                for (Empresa e : empresas) {
                    if (e.darNombre().equals(nombre)) {
                        return e;
                    }
                }
